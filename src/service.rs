...
use node_template_runtime::{self, GenesisConfig, opaque::Block, RuntimeApi, priceoracle};
		...
		AuthoritySetup = {
			|service: Self::FullService, executor: TaskExecutor, key: Option<Arc<Pair>>| {
				if let Some(key) = key {
					...
					let reg = service.config.custom.inherent_data_providers.register_provider(priceoracle::InherentDataProvider);
